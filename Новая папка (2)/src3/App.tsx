...
function App() {
    ...
    const addNewItem = (title: string) => {
        const newItemTodoId = v1()
        let newTodo: TodoListType = {id: newItemTodoId, title: title, filter: 'all'}
        setTodoList([newTodo, ...todoList])
        setTasks({...tasks, [newItemTodoId]: []})
    }

    const onChange = (title: string, taskId: string, todoListId: string) => {
        const todoListTasks = tasks[todoListId]
        tasks[todoListId] = todoListTasks.map(i => i.id === taskId ? {...i, title: title} : i)
        setTasks({...tasks})
    }

    return (
        <div className="App">
            <AddInputForm addTask={addNewItem} id={'kkk'}/>
